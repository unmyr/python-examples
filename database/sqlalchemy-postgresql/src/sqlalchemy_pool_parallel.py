...
class EngineHelper:
    ...
    def __init__(
        self, driver_name: str, application_name: str, poolclass=sqlalchemy.pool.QueuePool
    ) -> None:
        ...
        self.engine: sqlalchemy.engine.base.Engine = sqlalchemy.create_engine(
            database_url, poolclass=poolclass, connect_args={"application_name": application_name}
        )
...
def execute_select(
    cnt: int, engine: sqlalchemy.engine.base.Engine, application_name: str
) -> typing.List[typing.Tuple]:
    """Delay hello"""
    with engine.connect() as conn:
        logger.debug(__(f"CALL: {cnt}: conn.execute"))
        result = conn.execute(
            sqlalchemy.text(f"SELECT pg_sleep(2), {cnt}"), {"application_name": application_name}
        )
...
def call_engine_helper(
    ...
    poolclass: typing.Union[
        typing.Type[sqlalchemy.pool.QueuePool], typing.Type[sqlalchemy.pool.StaticPool]
    ],
) -> None:
    """Call engine helper"""
    total_tasks: int = 30
...
if __name__ == "__main__":
    if len(sys.argv) == 3:
        if sys.argv[1] in ["postgresql+pg8000", "postgresql+psycopg2"] and sys.argv[2] in [
            "StaticPool",
            "QueuePool",
        ]:
