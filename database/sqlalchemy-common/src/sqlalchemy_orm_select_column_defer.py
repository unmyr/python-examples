"""Example of sqlalchemy.orm.deferred."""
import json
import logging
import os
import sys
...
class FruitsMenu(Base):
    ...
    mod_time = sqlalchemy.orm.deferred(Column(DateTime, server_default=sqlalchemy.sql.func.now()))
...
def execute_query(
    ...
    """Execute query."""
    t_0 = time.time()
    count = session.query(sqlalchemy.func.count(FruitsMenu.id)).scalar()
    t_1 = time.time()
    ...
    t_2 = time.time()
    query_obj = session.query(FruitsMenu).filter(
        sqlalchemy.or_(FruitsMenu.name == "Apple", FruitsMenu.name == "Orange")
    )
    items = query_obj.all()
    ...
    result = {"statusCode": 200, "body": json.dumps(records)}
    t_4 = time.time()
    ...
    perf_list = (
        ...
        PerfInfo(
            round(t_3 - t_2, 3),
            "SELECT id, name, price FROM fruit_menu WHERE name='Apple' OR name = 'Orange'",
        ),
...
def main(driver_name: str) -> typing.Dict:
    ...
    try:
        ...
        perf_list_main = (
            PerfInfo(round(t_9 - t_0, 3), "Total"),
            ...
            PerfInfo(round(t_8 - t_7, 3), "Base.metadata.drop_all(engine)"),
            PerfInfo(round(t_9 - t_8, 3), "engine.dispose()"),
        )
        item: PerfInfo
        print("\n*** subtotal of main ***")
