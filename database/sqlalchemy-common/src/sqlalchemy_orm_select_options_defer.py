...
class Item:
    ...
    def __init__(self, row):
        print(f"item = {row}, type={type(row)}")
        self.id = row.id
        self.name = row.name
        self.price = row.price
        # if row
        # self.mod_time = row.mod_time
...
def execute_query(
    ...
    query_obj = (
        ...
        .options(sqlalchemy.orm.defer("mod_time"))
    )
    items = query_obj.all()
    ...
    records = [Item(item) for item in items]
    ...
    result = {"statusCode": 200, "body": records}
    t_4 = time.time()
