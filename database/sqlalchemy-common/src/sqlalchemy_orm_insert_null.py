    ...
    try:
        ...
        with engine.connect() as connection:
            with Session(bind=connection) as session:
                ...
                if count == 0:
                    session.add(DateTable(1, datetime.date.today()))
                    session.add(DateTable(2, sqlalchemy.null()))
                    session.add(DateTable(3, datetime.date(2021, 1, 1)))
                    session.add(DateTable(4, sqlalchemy.null()))
                    session.commit()

                items = (
                    session.query(DateTable).filter(DateTable.entry_date == sqlalchemy.null()).all()
                )
                for item in items:
                    print({"data_id": item.data_id, "entry_date": item.entry_date})
