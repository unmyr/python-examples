...
class FruitsMenu(Base):
    ...
    __table_args__ = (sqlalchemy.PrimaryKeyConstraint("id"), {"schema": "dummy_schema"})
...
def main(driver_name: str) -> None:
    ...
    with engine.connect() as conn:
        ...
        if driver_name == "sqlite":
            ...
            conn.execute(
                sqlalchemy.text("ATTACH DATABASE ':memory:' AS :schema"), {"schema": default_schema}
            )
        elif hasattr(engine.dialect, "has_schema"):
            if typing.cast(typing.Any, engine.dialect).has_schema(conn, default_schema):
                pass
            else:
                conn.execute(sqlalchemy.schema.CreateSchema(default_schema))
        ...
        with Session(bind=connection) as session:
            if session.query(FruitsMenu).count() == 0:
                session.bulk_save_objects(
                    [FruitsMenu("Apple", 10), FruitsMenu("Banana", 120), FruitsMenu("Orange", 110)]
                )
                session.commit()
...
def usage(command_name: str):
    """Usage."""
    print(
        f"usage: {command_name} " "{sqlite|postgresql+pg8000|postgresql+psycopg2}", file=sys.stderr
    )
