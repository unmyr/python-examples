...
def run(timeout: int) -> bool:
    ...
    params: typing.List[typing.Tuple[str, int]] = [("foo", 3), ("bar", 1), ("baz", 1)]
    for param in params:
    ...
    for i, future in enumerate(futures):
        try:
            ...
            logger.info(
                __('DONE: result(timeout={}): {}: ret="{}"', timeout, params[i], val)
            )
        except concurrent.futures.TimeoutError:
            # logger.exception(exc)
            logger.warning(
                __("FAIL: result(timeout={}): {}: timed out", timeout, params[i])
            )
