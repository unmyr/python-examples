...
class FooThread(threading.Thread):
    ...
    def __init__(self):
        ...
        self.sleep_time = 3
        print("%s:%s init()" % (str(datetime.datetime.today()), self.thread_name))
        threading.Thread.__init__(self)

    def run(self):
        print("%s:%s run()#begin" % (str(datetime.datetime.today()), self.thread_name))
        time.sleep(self.sleep_time)
        print("%s:%s run()#end" % (str(datetime.datetime.today()), self.thread_name))
