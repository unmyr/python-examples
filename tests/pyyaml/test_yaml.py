...
- # Explicit keys
  ...
  label: center/big""".strip()
        ...
        self.assertListEqual(
            new_dict, [{"x": 1, "y": 2, "r": 10, "label": "center/big"}]
        )

    def test_yaml_null(self):
        """Test datetime."""
        tests = ["canonical: ~", "english: null", "~: null key"]
        new_dict = yaml.safe_load("\n".join(tests))
        self.assertDictEqual(
            new_dict, {"canonical": None, "english": None, None: "null key"}
        )
    ...
    def test_yaml_datetime(self):
        ...
        self.assertEqual(
            d,
            datetime.datetime(
                2001, 12, 15, 2, 59, 43, 999999, tzinfo=datetime.timezone.utc
            ),
        )
