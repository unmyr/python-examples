...
class TestCalc(unittest.TestCase):
    ...
    def test_dict_no_child(self):
        ...
        self.assertDictEqual(new_dict, {"str-key": "VAL:STR"})

    def test_dict_child(self):
        """Test dict."""
        new_dict = rename_dict_keys_all({"Str-Key": {"CHILD-KEY": "child-val"}})
        self.assertDictEqual(new_dict, {"str-key": {"child-key": "CHILD-VAL"}})

    def test_dict_child_tuple(self):
        """Test dict."""
        org_dict = {"Tuple-Key": ("t1", "t2", {"FOO": "bar"})}
        new_dict = rename_dict_keys_all(org_dict)
        self.assertDictEqual(new_dict, {"tuple-key": ("T1", "T2", {"foo": "BAR"})})
