...
class TestCalc(unittest.TestCase):
    ...
    def test_str_num_rename_values(self):
        ...
        new_str1 = recursive(
            org_str1, value_func=lambda v: v.lower() if isinstance(v, str) else v
        )
        ...
        chk_num1 = 3
        new_num1 = recursive(
            org_num1, value_func=lambda v: v.lower() if isinstance(v, str) else v
        )
        assert new_num1 == chk_num1

        org_date1 = datetime.date(2017, 11, 12)
        chk_date1 = datetime.date(2017, 11, 12)
        new_date1 = recursive(org_date1, 0)
    ...
    def test_dict_rename_values(self):
        ...
        new_dict1 = recursive(
            org_dict1, value_func=lambda v: v.lower() if isinstance(v, str) else v
        )
        self.assertDictEqual(new_dict1, chk_dict1)
        ...
        new_dict2 = recursive(
            org_dict2, value_func=lambda v: v.lower() if isinstance(v, str) else v
        )
    ...
    def test_list_dict_rename_keys(self):
        ...
        new_list1 = recursive(org_list1, value_func=str)
        self.assertListEqual(new_list1, chk_list1)
    ...
    def test_tuple_rename_values(self):
        ...
        new_tuple1 = recursive(
            org_tuple1, value_func=lambda v: v.lower() if isinstance(v, str) else v
        )
